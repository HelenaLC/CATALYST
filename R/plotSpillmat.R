...
plotSpillmat <- function(bc_ms, SM, out_path=NULL, name_ext=NULL) {
    ...
    p <- ggplot(df, aes_string(x="c1", y="c2", fill="spill")) + 
        ...
        scale_fill_gradientn(colours=pal, guide=FALSE, 
            limits=c(0, 5), na.value="lightgrey") +
        geom_text(aes_string(label="spill_labs"), col="black", size=2.5) +
        annotate("text", rep(n+1.15, n), 1:n, label=rev(row_labs), 
            fontface="bold", size=2, col=rev(lab_cols)) +
        ...
        theme(panel.grid.major=element_blank(), panel.border=element_blank(),
            axis.text.x=element_text(vjust=.5, hjust=1, size=8, angle=90),
            axis.text.y=element_text(vjust=.5, hjust=1, size=8, color=rev(lab_cols)))
