...
Multiple FCS files can be concatenated via `concatFCS`. If `out_path=NULL` (the default), the function will return a single `flowFrame` containing the measurement data of all files. Otherwise, an FCS 3.0 standard file of the concatenated data will be written to the specified location.
...
trimVal <- estTrim(x=re, min=0.04, max=0.12, step=0.01)
```
...
bw <- .25; n <- 64
smoothScatter(ss_t[, which],           nbin=n, bandwidth=bw, colramp=cols, main='Single stains')
smoothScatter(ss_comped_t[, which],    nbin=n, bandwidth=bw, colramp=cols, main='Compensated')
smoothScatter(cells_t[, which],        nbin=n, bandwidth=bw, colramp=cols, main='Multiplexed cells')
smoothScatter(comped_cells_t[, which], nbin=n, bandwidth=bw, colramp=cols, main='Compensated')
```
