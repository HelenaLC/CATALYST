...
For the estimation of cutoff parameters we consider yields upon debarcoding as a function of the applied cutoffs. Commonly, this function will be characterized by an initial weak decline, where doublets are excluded, and subsequent rapid decline in yields to zero. Inbetween, low numbers of counts with intermediate barcode separation give rise to a plateau. To facilitate robust estimation, we fit a three-parameter log-logistic function [@Finney] to the yields function with the `LL.3` function of the `r CRANpkg("drc")` R package [@drc]. As an adequate cutoff estimate, we target a point that marks the end of the plateau regime and on-set of yield decline to appropriately balance confidence in barcode assignment and cell yield. 

By default, we compute this point as the first local minimum of the fifth derivative (`deriv = 5`) to assure the selected cutoff lies well before the yield functions last inflection point, and that its application does not cause a significant decline in yield. Generally, higher order derivatives will give more liberal, low cutoff estimates; and lower order derivatives will render more stringent, higher estimates (see Figure \@ref(fig:estCutoffs)). Thence, depending on the overall doublet-singlet separation, and how bimodal the count distribution is, another derivate may provide a better estimate.

![(\#fig:estCutoffs) Impact of the derivative order used for cutoff estimation. Top: Three-parameter log-logistic function fit of yields as a function of applied separation cutoffs. Bottom: First through fifth derivative of the fitted function. Higher order derivatives will yield more liberal-, lower order derivatives more stringent cutoff estimates.](estCutoffs.png)
...
plotYields(x=re, which="A1")
plotYields(x=estCutoffs(x=re0, deriv=6), which="A1")
```
...
where $\text{SM}^{-1}$ is termed compensation matrix ($\text{CM}$).

Because any signal not in a single stain experiment’s primary channel $j$ results from channel crosstalk, each spill entry $s_{ij}$ can be approximated by the slope of a linear regression with channel $j$ signal as the response, and channel $i$ signals as the predictors, where $i\in w_j$. `r Rpackage("CATALYST")` offers two alternative ways for spillover estimation, summarized in Figure \@ref(fig:methods).
...
![(\#fig:methods) Population versus single-cell based spillover estimation.](methods.png)
...
![(\#fig:interactions) Heat map of spill expected interactions. These are considered by the default method of computeSpillmat.](interactions.png)
