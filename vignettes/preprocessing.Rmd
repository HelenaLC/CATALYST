...
```{r eval = FALSE}
plotYields(sce, which = c("C1", 0), plotly = FALSE)
```

```{r plotYields, echo = FALSE, fig.width = 7, fig.height = 3.5}
ps <- plotYields(sce, which = c("C1", 0), plotly = FALSE); ps[[1]]; ps[[2]]
```
...
![(\#fig:interactions) Heatmap of spill expected interactions. These are considered by the default method of <i>computeSpillmat</i>.](interactions.png){width="80%"}
...
<!--Otherwise, compensated data will be written to the specified location as FCS 3.0 standard files. Multiple datasets may be corrected based on the same spill estimates if the input `x` is a character string specifying the location of the FCS files to be compensated.-->

```{r compCytof, message = FALSE}
# construct SCE of multiplexed cells
data(mp_cells)
sce <- fcs2sce(mp_cells, by_time = FALSE)
# compensate using `flowCore` & NNLS-method
comped_flow <- compCytof(sce, sm, method = "flow")
comped_nnls <- compCytof(sce, sm, method = "nnls")
```

```{r echo = FALSE, message = FALSE, results = "hide"}
cf <- 20
es_t <- assay(sce, "exprs")
es_comped_flow <- assay(comped_flow, "exprs_comped")
es_comped_nnls <- assay(comped_nnls, "exprs_comped")
```
...
bw <- 0.2; n <- 100
smoothScatter(es_t[chs, ], 
    nrpoints = 0, nbin = n, bandwidth = bw, 
    colramp = cols, main = "uncompensated")
smoothScatter(es_comped_flow[chs, ], 
    nrpoints = 0, nbin = n, bandwidth = bw, 
    colramp = cols, main = "'flowCore' compensated")
smoothScatter(es_comped_nnls[chs, ], 
    nrpoints = 0, nbin = n, bandwidth = bw, 
    colramp = cols, main = 'NNLS compensated')
```
