...
To this end, we fit a three-parameter log-logistic function [@Finney] to the yields function with the `LL.3` function of the `r CRANpkg("drc")` R package [@drc]. As an adequate cutoff estimate, we compute the first minimum of the forth derivative, which approximately marks the end of the plateu regime and on-set of yield decline (see figure below).
...
f3 <- function(x, b, c, d, e) (b*(c-d)*e^b*x^(-3+b)*((2-3*b+b^2)*e^(2*b)-4*(-1+b^2)*e^b*x^b+(2+3*b+b^2)*x^(2*b)))/(e^b+x^b)^4
...
x <- seq(0,100,.01); b <- 25; c <- 0; d <- 1; e <- 50
df <- data.frame(x, f0(x, b, c, d, e),
    f1(x, b, c, d, e)*(1/max(abs(f1(x, b, c, d, e)))),
    f2(x, b, c, d, e)*(1/max(abs(f2(x, b, c, d, e)))),
    f3(x, b, c, d, e)*(1/max(abs(f3(x, b, c, d, e)))),
    f4(x, b, c, d, e)*(1/max(abs(f4(x, b, c, d, e)))))
colnames(df) <- c("x", "f(x)", "f'(x)", "f''(x)", "f'''(x)", "f''''(x)")
df <- reshape2::melt(df, id.var="x")
root <- which(diff(sign(diff(f4(x, b, c, d, e)))) == 2) + 1
ggplot(df, aes(x=x, y=value)) + 
    ggtitle("The four-parameter log-logistic function") +
    geom_hline(yintercept=0, lty=1, size=.25, color="grey") +
    geom_vline(xintercept=50, lty=1, size=.25, color="grey") +
    geom_vline(xintercept=x[root[1]], lty=2, size=.5, color="red3") +
    facet_grid(variable~.) + geom_line(size=.5) + 
    scale_x_continuous(limits=c(0,100), breaks=NULL) +
    scale_y_continuous(limits=c(-1,1), breaks=0) +
    labs(x=NULL, y=NULL) + theme_classic() + theme(
        axis.text=element_text(size=8),
        aspect.ratio=1/5, plot.title=element_text(hjust=.5, size=10),
...
Because any signal not in a single stain experiment’s primary channel $j$ results from channel crosstalk, each spill entry $s_{ij}$ can be approximated by the slope of a linear regression with channel $j$ signal as the response, and channel $i$ signals as the predictors, where $i\in w_j$.

The default method estimates spillover as the median ratio between the unstained spillover receiving and stained spillover emitting channel in the corresponding single-positive populations:

$$s_{ij} = med(m_j^+/m_i^+)$$

As in conventional flow cytometry, `method = "classic"` calculates $s_{ij}$ as the slope of a line through the medians (or trimmed means) of stained and unstained populations, $m_j^+$ and $m_i^+$, respectively. The medians (or trimmed means) computed from events that are i) negative in the respective channels; and, ii) not assigned to interacting channels; and, iii) not unassigned, $m_j^-$ and $m_i^-$, respectively, are subtracted as to account for background according to:
...
The list of mass channels considered to potentionally contain isotopic contaminatons is given below.
...
Alternatively, `interactions = "all"` will compute a spill estimate for all $n\cdot n-n$ interactions, where $n$ denotes the number of single-color controls. Estimates falling below the threshold specified by `th` will be set to zero. Lastly, the SM’s diagonal entries $s_{ii}$ are set to 1 so that spill is relative to the total signal measured in a given channel.
