...
```{r plotPbExprs-1, fig.width = 12, fig.height = 5}
...
```{r plotPbExprs-2, fig.width = 12, fig.height = 4}
...
```{r plotPbExprs-3, fig.width = 12, fig.height = 4}
...
```{r plotPbExprs-4, fig.width = 12, fig.height = 4}
...
Alternatively, dimension reductions can be computed using one of `r BiocStyle::Biocpkg("scrapper")`'s `runX.se` functions (`X = "Tsne", "Umap", ...`). Note that, by default, `scrapper` expects expression values to be stored in the `logcounts` assay of the SCE; specification of `assay.type = "exprs"` is thus required:

```{r runUMAP-scrapper, eval = FALSE}
# run UMAP on principal components computed from all features
sce <- runPca.se(sce, features=rownames(sce), assay.type="exprs")
sce <- runUmap(sce, reddim.type="PCA")
```
...
```{r plotPbExprs-custom, fig.width = 8, fig.height = 2}
