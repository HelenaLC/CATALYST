...
#' @rdname estTrim
#' @title Estimation of trim value
#' 
...
#' @param min,max,step specify sequence of trim values for which compensation should be evaluated.
#' @param out_path specifies in which location output plot is to be generated. Defaults to NULL.
#' @param name_ext a character string. If specified, will be appended to the output plot's name. Defaults to NULL.
#' 
#' @return
#' For each value along \code{seq(min, max, step), \code{computeCompat} will be called with \code{method = "mean"} 
#' and the respective trim parameter. Returned will be the value that minimizes the sum over squared population-wise
#' median counts across all barcodes after compensation.}
#' 
#' @examples
#' data(ss_beads)
...
setMethod(f="estTrim", 
          ...
          definition=function(x, min = 0.05, max = 0.20, step = 0.01, out_path = NULL, name_ext = NULL) {
              ...
              for (trim in trim_vals) {
                  ...
                  rss <- rbind(rss, data.frame(m=mean((df$m[df$trim == trim])^2), trim=trim))
              }
              opt <- rss$trim[which.min(rss$m)]
              
              p <- ggplot() + 
                  geom_vline(aes_string(xintercept=opt), lty=3) +
                  geom_jitter(data=df, aes_string(x="trim", y="m"), height=0, width=step/5, 
                              col="mediumblue", alpha=.2, stroke=.5, size=4) + 
